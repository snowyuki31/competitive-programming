...
int main()
{
    ...
    rep(i, 5) rep(j, 5) if(i < j){
        set<ll> st;
        rep(k, 5) if(i != k and j != k) st.insert(A[k]);
        if(len(st) == 1){
            dp[A[i]]++, dp[A[j]]++;
            ans = 1;
            break;
    ...
    rep(_, 5) deq.pop_front();

    rep(i, N - 2) {
        map<ll, ll> tmp;
        set<ll> tp;
        rep(_, 3) ++tmp[deq.front()], tp.insert(deq.front()), deq.pop_front();
        if(len(tmp) == 1){
            ++ans;
            continue;
        }
        ...
        bool plus = false;
        for(auto [key, val] : tmp) if(dp[key] + val == 3) plus = true;
