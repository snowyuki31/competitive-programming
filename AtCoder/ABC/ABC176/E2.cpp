...
int main()
{
    ...
    ll rowmx = -1;
    ll rmx = 0;
    for(auto [key, val] : rows) chmax(rmx, val);
    set<ll> ROW;
    for(auto [key, val] : rows) if(val == rmx) ROW.insert(key);

    //col最大
    ll colmx = -1;
    ll cmx = 0;
    for(auto [key, val] : cols) chmax(cmx, val);
    set<ll> COL;
    for(auto [key, val] : cols) if(val == cmx) COL.insert(key);

    ll ans = rmx + cmx;

    ll tmp = 0;
    for(auto [pos, num] : bomb)
    {
        auto [x, y] = pos;
        if(ROW.count(x) and COL.count(y)) ++tmp;
    }
    if(tmp >= len(ROW) * len(COL)) --ans;
