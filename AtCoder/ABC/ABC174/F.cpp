...
int main()
{
    ...
    vec lastAppeared(5e5 + 100, -1);

    rep(i, N) lastAppeared[A[i]] = i;

    BinaryIndexedTree<ll> BIT(N);
    for(auto a : lastAppeared) if(a != -1) BIT.add(a, 1);

    vec ans(Q);

    ll prev = -1;
    rep(i, N){
        auto [R, L, idx] = Query[i];
        if(R > prev){
            lastAppeared[A[R]] = R;
            BIT.add(lastAppeared[R], 1);
            if(prev != -1) BIT.add(prev, -1);
            prev = R;
        }
