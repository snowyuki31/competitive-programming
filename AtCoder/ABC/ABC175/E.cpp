...
int main()
{
    ...
    vec3 res(4, vec2(R + 1, vec(C + 1, 0)));

    rep1(j, R) rep1(k, C) {
        //変化なし
        rep(i, 4) chmax(res[0][j][k], res[i][j - 1][k]);
        rep(i, 4) chmax(res[i][j][k], res[i][j][k - 1]);

        //取る場合
        rrep(i, 4) if(i) chmax(res[i][j][k], res[i - 1][j][k] + mx[j][k]);
    }

    //for(auto r : res[3]) print(r);

    ll ans = 0;
    rep(i, 4) chmax(ans, res[i][R][C]);
    print(ans);
