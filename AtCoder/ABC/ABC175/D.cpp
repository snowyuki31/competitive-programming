...
int main()
{
    ...
    rep1(i, N) G.add_arrow(0, i, 0), R.add_arrow(0, i, 0);

    rep(i, N) {
        G.add_arrow(i + 1, X[i], 1);
        R.add_arrow(i + 1, X[i], -(C[X[i] - 1]));
    }

    vec dist = G.bellman_ford(0);
    vec score = R.bellman_ford(0);
