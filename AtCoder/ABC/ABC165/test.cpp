...
pair<vector<ll>, vector<ll>> dfs_on_tree(const Graph &g, ll root){
    vector<ll> depth(g.size(), 0);
    vector<ll> subtree_size(g.size(), 0);

    ll d = 0, par = -1, now = root;
    auto dfs = [&](auto &&self) -> void{
        depth[now] = d;
        ll prev = par;
        for(const auto &e : g[now]) if(e.to != par){
            now = e.to, par = e.from, ++d;
            self(self);
            now = e.from, par = prev, --d;
        }
        subtree_size[now] = 1;
        for(const auto &e : g[now]) if(e.to != par) subtree_size[now] += subtree_size[e.to];
    };
...
int main()
{
    ...
    auto [depth, subtree] = dfs_on_tree(G, 0);

    priority_queue<ll> pq;
    rep(i, n) pq.push(depth[i] - subtree[i] + 1);
    ll ans = 0;
    rep(i, k) ans += pq.top(), pq.pop();
    print(ans);
