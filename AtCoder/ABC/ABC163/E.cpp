...
int main()
{
    ...
    ll ans = 0;
    rep(x, N + 1) rep(y, N + 1) if(x + y >= 1 and x + y <= N) {
        auto [val, idx] = A[x + y - 1];
        if(y) chmax(f[x][y], f[x][y - 1] + val * abs(N - y - idx));
        if(x) chmax(f[x][y], f[x - 1][y] + val * abs(x - 1 - idx));
        if(x + y == N) chmax(ans, f[x][y]);
    }
