...
int main()
{
    ...
    rep(i, H) sort(ALL(XL[i])), sort(ALL(XB[i]));
    rep(j, W) sort(ALL(YL[j])), sort(ALL(YB[j]));

    map<P, ll> block;
    for(auto b : blocks) block[b]++;
    ...
    rep(i, H) rep(j, W){
        ...
        //X方向のライト
        auto L = lower_bound(ALL(XL[i]), j);
        //X方向のブロック
        auto B = lower_bound(ALL(XB[i]), j);
        ...
        //X方向のライト
        auto LL = lower_bound(ALL(YL[j]), i);
        //X方向のブロック
        auto BB = lower_bound(ALL(YB[j]), i);
