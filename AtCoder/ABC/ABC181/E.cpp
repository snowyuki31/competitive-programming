...
int main()
{
    ...
    vec d1, d2;
    rep(i, N - 1) if(i % 2 == 0) d1.pb(abs(H[i] - H[i + 1]));
    rep(i, N) if(i % 2) d2.pb(abs(H[i] - H[i + 1]));
    ...
    rep(i, N) if(i % 2 == 0){
        ...
        //[0, i/2)のsum1の和
        //[i/2, ...)のsum2の和
        ll sum = sum1[i / 2] + sum2.back() - sum2[i / 2];
        //print(sum);

        //H[i]の選び方
        ll mn = INF;
        auto itr = lower_bound(ALL(W), H[i]);
        chmin(mn, abs(*itr - H[i]));
        if(itr != W.begin()){
            --itr;
            chmin(mn, abs(*itr - H[i]));
        }
