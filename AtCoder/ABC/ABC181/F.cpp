...
int main()
{
    ...
    auto f = [&](double r)->bool {
        ...
        for(ll i = -100; i <= 100; ++i) ok[i] = true;
        
        ll prev = -500;
        for(auto &[x, ys] : mp){
            ll d = x - prev;

            //距離が直径より大きく、半径より大きい移動可能領域があれば全部移動可能
            if(d > r){
                ll mx = 0;
                ll ct = 0;
                for(ll i = -100; i <= 100; ++i){
                    if(!ok[i]){
                        if(ct > 0) chmax(mx, ct + 1);
                        ct = 0;
                    }
                    else ++ct;
                }
                if(ct > 0) chmin(mx, ct + 1);

                if(mx >= r) for(ll i = -100; i <= 100; ++i) ok[i] = true;
            }

            rep(i, len(ys) - 1){
                //まずys[i] ~ ys[i + 1]が全部okなら通れる
                bool pass = true;
                for(ll j = ys[i] + 1; j < ys[i + 1]; ++j){
                    if(!ok[j]) pass = false;
                }

                //半径より小さい領域は通れない
                if(ys[i + 1] - ys[i] < r) pass = false;

                //通れないなら全部falseに
                if(!pass){
                    for (ll j = ys[i]; j <= ys[i + 1]; ++j) ok[j] = false;
                }
                //通れるなら点だけfalseに
                else{
                    ok[ys[i]] = false;
