...
int main()
{
    ...
    SegmentTree<ll> segtree(2e5 + 10, [](ll x, ll y) { return min(x, y); }, INF);

    vec A(2e5 + 10), B(2e5 + 10);

    //幼稚園iのhighestの値, 人数
    vector<multiset<ll>> highest(2e5 + 10);

    rep(i, N){
        cin >> A[i] >> B[i];
        --B[i];

        highest[B[i]].insert(A[i]);
    }

    rep(i, 2e5 + 10){
        if(highest[i].empty()) segtree.set(i, INF);
        else segtree.set(i, *highest[i].rbegin());
    }
    segtree.build();
    ...
    rep(i, Q){
        ...
        auto itr = highest[prevK].lower_bound(A[C]);
        highest[prevK].erase(itr); //元の所属から削除

        highest[D].insert(A[C]); //今の所属に入れる

        ll prevHigh = segtree[prevK]; //元の所属のhighest (Segtree上)

        if(highest[prevK].empty()){
            segtree.update(prevK, INF);
        }else if(prevHigh != *highest[prevK].rbegin()){
            segtree.update(prevK, *highest[prevK].rbegin());
        }

        if(segtree[D] != *highest[D].rbegin()){
            segtree.update(D, *highest[D].rbegin());
        }

        print(segtree.query(0, 2e5));
