...
int main()
{
    ...
    rep(i, N) {
        ...
        ll now = a * pow(10, 9) + b * pow(10, 9 - dim);
        //debug(now);
        ll two = 0, five = 0;
        while (now % 2 == 0) now /= 2, ++two;
        while (now % 5 == 0) now /= 5, ++five;
        mp[{two, five}]++;

    }

    vector<pair<P, ll>> ls;
    for (auto [p, ctp] : mp) ls.pb({ p, ctp });
    ll L = len(ls);
    ll ans = 0;

    //for (auto [p, l] : ls) debug(p), debug(l);

    rep(i, L) rep(j, L) if(i <= j){ 
        auto [pt, pf] = ls[i].first;
        auto [qt, qf] = ls[j].first;
        ll ctp = ls[i].second;
        ll ctq = ls[j].second;


        if ((pt + qt >= 18) and (pf + qf >= 18)) {
            //debug(P({ pt, qt }));
            //debug(P({ pf, qf }));
            if (i == j) {
                ans += ctp * (ctp - 1) / 2;
