...
int main()
{
    ...
    auto sor = [&](const string &x, const string &y) {
        ll L = len(x);
        string tp = "";
        rep(i, L){
            if(x[i] == '0' and y[i] == '0') tp += '0';
            else tp += '1';
        }
        return tp;
    };

    auto popcount = [&](const string &x) {
        ll res = 0;
        for(auto c : x) res += c == '1';
        return res;
    };
    ...
    SegmentTree<string> segtree(N, sor, init);
    ...
    rep(_, Q){
        ...
        cin >> x >> y;
        --x, --y;
        string ans = segtree.query(x, y + 1);
        print(popcount(ans));
    }
