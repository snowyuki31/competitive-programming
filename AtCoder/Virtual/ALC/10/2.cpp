    ...
    while(!que.empty()){
        ...
        for(auto e : G[now]){
            if(color[e.to] == -1){
                ...
                if(abs(color[e.to] - c) != 1) drop(-1);
            }
        }
    }

    ll res = 0;
    rep(i, N) chmax(res, color[i]);
...
int main()
{
    ...
    Graph G(N);
    rep(i, N) rep(j, N) if (i < j) if (S[i][j] == '1') add_edge(G, i, j);

    ll ans = 0;
    rep(i, N) chmax(ans, bfs(G, N, i));
    print(ans);
