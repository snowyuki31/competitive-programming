...
struct Compression {
    ...
    vector<T> get(const vector<T> &v) const{
        vector<T> ret;
...
int main()
{
    ...
    Compression<ll> compress(S);
    compress.build();
    vec X = compress.get(S);

    ll ans = 0;

    SegmentTree<ll> segtree(N + 1, [&](ll x, ll y){return x + y;}, 0);
    segtree.build();

    rep(i, len(X)){
        segtree.update(X[i], segtree[X[i]] + 1);
        ans += segtree.query(0, X[i] + 1) - 1;
    }
