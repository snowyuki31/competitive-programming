...
int main()
{
    ...
    vector<vector<vector<mint>>> f(L + 1, vector<vector<mint>>(D, vector<mint>(2, 0)));
    f[0][0][0] = 1;

    rep1(i, L) rep(j, D){
        ll num = N[i - 1] - '0';
        f[i][(j + num) % D][0] += f[i - 1][j][0];
        rep(k, num) f[i][(j + k) % D][1] += f[i - 1][j][0];
        rep(k, 10) f[i][(j + k) % D][1] += f[i - 1][j][1];
    }
    print(f[L][0][0] + f[L][0][1] - 1);
