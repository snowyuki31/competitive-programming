...
int main()
{
    ...
    //start :: N : type A, N + 1 : type B, N + 2 : type C
    //goal :: N + 3 : type A, N + 4 : type B, N + 5: type C
    ll AB, AC, BC;
    cin >> AB >> AC >> BC;

    G.add_arrow(N, N + 4, AB);
    G.add_arrow(N, N + 5, AC);
    G.add_arrow(N + 1, N + 3, AB);
    G.add_arrow(N + 1, N + 5, BC);
    G.add_arrow(N + 2, N + 3, AC);
    G.add_arrow(N + 2, N + 4, BC);

    string S;
    cin >> S;
    rep(i, N){
        G.add_arrow(i, N + S[i] - 'A', 0);
        G.add_arrow(N + S[i] - 'A' + 3, i, 0);
    }
