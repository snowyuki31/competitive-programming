...
int main()
{
    ...
    rep(i, M){
        ...
        --u, --v;
        add_edge(Yen, u, v, a);
        add_edge(Snuke, u, v, b);
    }

    vec go = dijkstra(Yen, s);
    vec re = dijkstra(Snuke, t);

    vec exchange(N);
    rep(i, N) exchange[i] = go[i] + re[i];

    rrep(i, N) if(i >= 1) chmin(exchange[i - 1], exchange[i]);

    rep(i, N) print(ll(1e15) - exchange[i]);
