...
int main()
{
    ...
    while(!que.empty()){
        ...
        if(x == X and y == Y) drop(dist[x][y]);
        visit.insert({x, y});

        vector<P> next = {{x + 1, y + 1}, {x, y + 1}, {x - 1, y + 1}, {x + 1, y}, {x - 1, y}, {x, y - 1}};

        for(auto [nx, ny] : next){
            if(nx < 0 or ny < 0 or nx >= 600 or ny >= 600) continue;
