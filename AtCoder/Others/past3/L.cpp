...
int main()
{
    ...
    SegmentTree<ll> f(N, [&](ll x, ll y){return max(x, y);}, -INF);
    SegmentTree<ll> b(N, [&](ll x, ll y){return max(x, y);}, -INF);
    ...
    rep(i, N){
        ...
        rep(j, K){
            ...
            mp[t] = i;
            x[i].push_back(t);
            if(j == 0) f.set(i, t), x[i].pop_front();
            if(j == 1) b.set(i, t), x[i].pop_front();
        }
    }
    ...
    rep(i, M){
        ...
        if(a == 1){
            ...
            ll next = x[idx].empty() ? -1 : x[idx].front();
            if(!x[idx].empty()) x[idx].pop_front();
