...
int main()
{
    ...
    rep(i, L + 9){
        ll obs = 0;
        obs = x.count(i) ? T3 : 0;
        if(i >= 1) chmin(dp[i], dp[i - 1] + T1 + obs);

        //以前で行動2の場合
        if(i >= 2) chmin(dp[i], dp[i - 2] + T1 + T2 + obs);

        //以前で行動3の場合
        if(i >= 4) chmin(dp[i], dp[i - 4] + T1 + 3 * T2 + obs);
    }

    ll ans = dp[L];
    rep1(i, 4){
        ll now = L - i;
        ll obs = x.count(now) ? T3 : 0;
        ll tp = dp[now] + obs + T1 / 2 + T2 * (i - 1) / 2;
        chmin(ans, tp);
    }
