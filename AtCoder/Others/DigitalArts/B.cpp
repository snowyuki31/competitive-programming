...
int main()
{
    ...
    cin >> t;
    s = t;
    if(s == "a" or s == "zzzzzzzzzzzzzzzzzzzz") drop("NO");

    sort(ALL(s));
    if(s.back() != 'a' and len(s) <= 19) --s.back(), drop(s + "a");

    if(s.back() == 'a') s.pop_back(), s.pop_back(), drop(s + "b");
    ...
    rep(i, len(t)) rep(j, len(t)) if(i < j){
        if(t[i] != t[j]){
            swap(t[i], t[j]);
            drop(t);
        }
