...
int main()
{
    ...
    vec res(N);
    rep(i, N) res[i] = pos[i].second;

    UnionFind UF(N);

    segtree<S, op, e> segM(res);
    segtree<S, op2, e2> segm(res);

    rep(i, N){
        ll mx = segM.prod(i, N);
        ll mn = segm.prod(0, i);

        if(mx > res[i]) UF.merge(i, index[mx]);
        if(mn < res[i]) UF.merge(i, index[mn]);
    }
