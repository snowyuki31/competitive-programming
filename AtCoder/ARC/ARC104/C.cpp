...
int main()
{
    ...
    //逆行してる場合はあり得ない
    for(auto [A, B] : info){
        if(A != -1 and B != -1 and A >= B) drop("No");
    }
    //階で乗り降りした人が複数いる場合はありえない
    vec in(2 * N + 1);
    vec out(2 * N + 1);
    for(auto [A, B] : info){
        if(A != -1){
    ...
    for(auto [A, B] : info){
        ...
        ll C = B - A - 1;
        for(ll i = A; i <= B; ++i){
            if(res[i] != -1 and res[i] != C) drop("No");
            res[i] = C;
        }
    }

    //C条件が確定しているものから確定させていく
    vector<P> ans = info;
    ll ct = 0;
    ll cti = 0;
    for(auto& [A, B] : ans){
