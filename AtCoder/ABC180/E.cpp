...
int main()
{
    ...
    //集合Sになって、最後がxのときの最小値
    vec2 F(1 << (N + 1), vec(N + 1, INF));

    auto rec = [&](auto&& self, ll bit, ll v)->ll{
        if(F[bit][v] != INF) return F[bit][v];

        if(bit <= 1) return F[bit][v];
        ...
        rep(u, N) if(u) {
            ...
            ll dist = abs(xn - x) + abs(yn - y) + max(0LL, zn - z);
            chmin(res, self(self, prev, u) + dist);
        }
