...
int main()
{
    ...
    rep1(i, N){
        ...
        rep(j, V + 1) if(j + dist <= V){
            chmin(f[i][j], f[i - 1][j + dist]);
            chmin(f[i][min(V, j + v)], f[i - 1][j + dist] + w);
        }
    }

    ll ans = INF;
    ll last = get<0>(X.back());

    rep(j, V) if(j >= L - last) chmin(ans, f[N][j]);
    print(ans == INF ? -1 : ans);
