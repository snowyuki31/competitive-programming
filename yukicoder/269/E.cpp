...
int main()
{
    ...
    while(T--){
        ...
        mint G = mint(AG) / mint(BG);
        mint C = mint(AC) / mint(BC);
        mint P = mint(AP) / mint(BP);

        mint ans = 1;
        //グーパーのみ
        ans -= modpow(G + P, N) - modpow(G, N) - modpow(P, N);
        //パーチョキのみ
        ans -= modpow(P + C, N) - modpow(P, N) - modpow(C, N);
        //チョキグーのみ
        ans -= modpow(C + G, N) - modpow(C, N) - modpow(G, N);
