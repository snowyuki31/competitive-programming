...
int main()
{
    ...
    rep1(i, N){
        ...
        //i番目を選ぶとき
        rep1(j, N){
            if(j % 2) chmax(dp[i][j], dp[i - 1][j - 1] + now);
            else chmax(dp[i][j], dp[i - 1][j - 1] - now);
        }
        //i番目を選ばないとき
        rep1(j, N) chmax(dp[i][j], dp[i - 1][j]);
    }

    ll ans = 0;
    for(ll j = 0; j <= N; ++j) chmax(ans, dp[N][j]);
    print(ans);
