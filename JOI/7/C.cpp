...
int main()
{
    ...
    rep(i, p){
        ...
        tmp[0] = i % p;
        tmp[1] = (tmp[0]) * i % p;
        for(ll k = 2; k < 32; ++k) tmp[k] = tmp[k - 1] * tmp[k - 1] %p;
        ll ans = 1;
        rep(k, 32) if(N & (1LL << k)) ans *= tmp[k], ans %= p;
        ++mp[ans];
    }

    ll ans = 0;
    for(auto& [x, ctx] : mp) for(auto& [y, cty] : mp) if(x <= y){
        if(mp.count((x + y) % p)) ans += ctx * cty * mp[(x + y) % p] * (x == y ? 1 : 2);
    }
    print(ans);
