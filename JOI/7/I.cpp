...
int main()
{
    ...
    rep(i, N){
        auto [a, b, idx] = pos[i];
        for(ll x = -D; x <= D; ++x) rep(y, D + 1) if(x * x + y * y <= D * D and x * x + y * y >= 1){
            if(a + x >= 0 and a + x <= 1000 and b + y <= 1000) if(coord[a + x][b + y] != -1) G.add_edge(idx, coord[a + x][b + y]);
        }
    }
    
    vec res = G.dijkstra(0);
    ll ans = 0;
    rep(i, N) ans += res[i] <= K and res[i] >= K - M + 1;
    print(ans);
