...
int main()
{
    ...
    rep(j, N) if(j > 0) if(es[0][j] != -1) G.add_arrow(0, 0 + j * N, es[0][j]);

    rep(i, N) rep(j, N) rep(k, N) if(es[i][j] != -1 and es[j][k] != -1){
        auto [ax, ay] = pos[i];
        auto [bx, by] = pos[j];
        auto [cx, cy] = pos[k];

        ll a2 = (ax - bx) * (ax - bx) + (ay - by) * (ay - by);
        ll b2 = (bx - cx) * (bx - cx) + (by - cy) * (by - cy);
        ll c2 = (cx - ax) * (cx - ax) + (cy - ay) * (cy - ay);
        ...
        //cout << "debug : " << i << " " << j << " " << k << '\n';
        //debug(res);
        G.add_arrow(i + j * N, j + k * N, es[j][k]);
    }

    vec res = G.dijkstra(0);

    ll ans = INF;
    rep(i, N) chmin(ans, res[i + 1 * N]);
    print(ans == INF ? -1 : ans);
