...
int main()
{
    ...
    vec2 people(H, vec(W));
    rep(i, H) rep(j, W) if(land[i][j] == -1) people[i][j] = 1;

    vec2 cuml(H + 1, vec(W + 1));
    vec2 cump(H + 1, vec(W + 1));

    rep1(i, H) rep1(j, W){
        cuml[i][j] += cuml[i][j - 1] + land[i - 1][j - 1];
        cump[i][j] += cump[i][j - 1] + people[i - 1][j - 1];
    }

    rep1(i, H) rep1(j, W) cuml[i][j] += cuml[i - 1][j], cump[i][j] += cump[i - 1][j];

    ll ans = INF;
    rep1(i, H) rep1(j, W) {
        if(i < a or j < b) continue;
        ll val = cuml[i][j] - cuml[i - a][j] - cuml[i][j - b] + cuml[i - a][j - b];
        ll num = cump[i][j] - cump[i - a][j] - cump[i][j - b] + cump[i - a][j - b];
        if(!num) chmin(ans, val);
    }
    print(ans);
