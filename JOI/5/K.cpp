...
struct RollingHash {
    ...
    RollingHash(const string& s) : hashed(s.size() + 1, 0), power(s.size() + 1, 0) {
        power[0] = 1;
...
int main()
{
    ...
    auto f = [&](ll L) {
        set<ll> st;
        rep(i, len(S)) if(i + L <= len(S)) st.insert(RHS.get(i, i + L));
        rep(i, len(T)) if(i + L <= len(T)) if(st.count(RHT.get(i, i + L))) return false;
        return true;
    };

    auto res = bisearch(0, min(len(S), len(T)) + 1, f);
    print(res.first);
