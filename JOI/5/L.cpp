...
int main()
{
    ...
    vec2 visited(N, vec(M));

    auto dfs = [&](auto &&self, ll i, ll j, ll dist)->void{
        if(res[i][j]) chmax(ans, dist);
        rep(x, 4){
            ll nx = i + dx[x], ny = j + dy[x];
            if(nx < 0 or nx >= N or ny < 0 or ny >= M) continue;
            if(!res[nx][ny]) continue;
            if(visited[nx][ny]) continue;

            visited[nx][ny] = 1;
            self(self, nx, ny, dist + 1);
            visited[nx][ny] = 0;
        }
    };

    rep(i, N) rep(j, M) if(res[i][j]){
        visited = vec2(N, vec(M, 0));
        visited[i][j] = 1;
        dfs(dfs, i, j, 1);
    }
    print(ans);
