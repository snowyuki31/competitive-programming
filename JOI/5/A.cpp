...
int main()
{
    ...
    ll N;
    cin >> N;
    vector<vector<ll>> res(N, vector<ll>(4));
    cin >> res;

    for(auto& r : res) rep(i, 3) --r[i];

    rep(i, N) if(res[i].back()) rep(j, 3) part[res[i][j]] = 1;

    rep(i, N) if (!res[i].back()) {
        ll ct = 0;
        rep(j, 3) ct += part[res[i][j]] == 1;
        if(ct == 2) rep(j, 3) if(part[res[i][j]] != 1) part[res[i][j]] = 0;
    }

    for(auto p : part) print(p);
