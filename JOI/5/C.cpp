...
int main()
{
    ...
    rep(i, N) rep(j, N) if(i < j){
        ...
        Point x = Point(xj, yj) - Point(xi, yi);
        Point rot1 = Point(cos(PI / 4), sin(PI / 4));
        Point rot2 = Point(cos(-PI / 4), sin(-PI / 4));
        rot1 /= sqrt(2), rot2 /= sqrt(2);

        Point a = x * rot1 + Point(xi, yi), b = x * rot2 + Point(xi, yi);

        ll ax = a.real() + EPS, ay = a.imag() + EPS;
        ll bx = b.real() + EPS, by = b.imag() + EPS;

        if (ls.count({ax, ay}) and ls.count({bx, by}))
        {
            chmax(ans, abs(x) * abs(x) / 2);
        }
    }
