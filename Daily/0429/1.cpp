...
int main()
{
    ...
    vec cum = cumsum(A);
    unordered_multimap<bitset<40>, ll> res;

    rep1(i, N) rep1(j, N) if(i <= j){
        bitset<40> beauty = cum[j] - cum[i - 1];
        res.insert({beauty, 1});
    }

    rrep(i, 40){
        ll ct = 0;
        unordered_multimap<bitset<40>, ll> next;
        for(auto r : res) if(r.first[i]) next.insert(r);
        if(len(next) < K) continue;
        res = next;
    }

    bitset<40> ans = ~0;
    for(auto r : res) ans &= r.first;
