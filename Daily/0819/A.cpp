...
int main()
{
    ...
    vec ans(A.begin() + m, A.end());
    set<ll> used;
    vec out;
    for(auto a : ans)
    {
        if(!used.count(a)) out.pb(a), used.insert(a);
        else
        {
            while(out.back() != a) used.erase(out.back()), out.pop_back();
            out.pop_back(), used.erase(a);
        }
    }
    print(out);
