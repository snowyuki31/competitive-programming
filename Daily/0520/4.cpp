...
template< typename T >
vector<T> dijkstra(Graph &g, T s, vector<T>& A, vector<T>& point, bool restore = false){
...
int main()
{
    ...
    rep(_, M){
        ...
        --a, --b;
        add_arrow(G, a, b, c);
        add_arrow(Ginv, b, a, c);
    }

    vec point(N, 0);
    vec dist = dijkstra(G, 0LL, A, point);
    vec distinv = dijkstra(Ginv, 0LL, A, point);

    vec cost(N);
    rep(i, N){
        if(dist[i] == INF or distinv[i] == INF){
            cost[i] = -1;
            continue;
        }
        cost[i] = dist[i] + distinv[i];
    }

    ll ans = 0;
    rep(i, N){
        if(cost[i] == -1) continue;
        chmax(ans, (T - cost[i]) * A[i]);
    }
