...
int main()
{
    auto convert = [&](string X) {
        ll H = stoi(X.substr(0, 2));
        ll M = stoi(X.substr(3, 2));
        ll S = stoi(X.substr(6, 2));
        ll m = stoi(X.substr(9, 3));
        return H * 60 * 60 * 1000 + M * 60 * 1000 + S * 1000 + m;
    };
    ...
    rep(i, N){
        string S, E;
        cin >> S >> E;
        tm.pb({convert(S), convert(E)});
        if(convert(S) >= convert(E)){
            rev = {convert(S) - 1000, convert(E) + 1000};
        }
    }
