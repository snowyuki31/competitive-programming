...
struct cumsum2D{
    ...
    cumsum2D(vector<vector<ll>>& grid) {
        ll H = len(grid), W = len(grid[0]);
        cum.resize(H + 1, vector<ll>(W + 1));
        for(int i = 1; i <= H; ++i) for(int j = 1; j <= W; ++j) cum[i][j] += cum[i][j - 1] + grid[i - 1][j - 1];
        for(int j = 1; j <= W; ++j) for(int i = 1; i <= H; ++i) cum[i][j] += cum[i - 1][j];
    }

    //i in [0, H), j in [0, W), 左上隅(i1, j1) 右下隅(i2, j2)内の領域の和
    ll getsum(ll i1, ll j1, ll i2, ll j2){
        return cum[i2 + 1][j2 + 1] + cum[i1][j1] - cum[i2 + 1][j1] - cum[i1][j2 + 1];
    }
};
...
int main()
{
    ...
    rep1(x, H) rep1(y, W){
        rep(i, H) rep(j, W){
            if(i + x - 1 >= H or j + y - 1 >= W) continue;
            ll sum = cumsum.getsum(i, j, i + x - 1, j + y - 1);
            if(sum == 0) chmax(ans, x * y);
        }
    }
    print(ans);
