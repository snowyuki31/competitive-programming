...
template< typename T >
vector<vector<T>> matadd(const vector<vector<T>> &A, const vector<vector<T>> &B){
...
vector<vector<T>> matmul(const vector<vector<T>> &A, const vector<vector<T>> &B){
    ...
    for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j){
            for (int k = 0; k < B.size(); ++k) res[i][j] += A[i][k] * B[k][j];
    }
    return res;
}
...
struct Matrix{
    vector<vector<T>> mat;
    Matrix(int n = 1, int m = 1, T v = 0) : res(n, vector<T>(m, v)) {}
    void init(int n, int m, T v = 0) { val.assign(n, vector<T>(m, v)); }
    void unit(int n, int m) { init(n, m, 0); }
};
...
int main()
{
    ...
    vector<vector<mint>> res(M, vector<mint>(M, 0));
    rep(i, M) res[i][i] = 1;
