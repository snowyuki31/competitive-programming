...
int main()
{
    ...
    vec2 what(100000 + 10, vec(4));
    vector<ll> rating;
    for(auto [R, H] : user) ++what[R][H - 1], rating.pb(R);
    sort(ALL(rating));

    for(auto [R, H] : user){
        --H;
        ll Win = lower_bound(ALL(rating), R) - rating.begin();
        ll Lose = rating.end() - upper_bound(ALL(rating), R);
        ll Draw = 0;
