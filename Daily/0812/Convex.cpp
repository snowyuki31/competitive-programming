...
{  
    ...
    auto res = ConvexHull(X);
    deque<Point> deq(ALL(res));
    
    Point mn(INF, INF);
    for(auto r : res){
        if(r.imag() < mn.imag()) mn = r;
        if(r.imag() == mn.imag() and r.real() < mn.real()) mn = r;
    }

    while(deq.front() != mn) deq.push_back(deq.front()), deq.pop_front();

    print(len(deq));
     for(auto r  : deq)  cout  << ll(r.real()) << " " << ll(r.imag()) << '\n';
