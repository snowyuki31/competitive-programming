...
int main()
{
    ...
    vector<mint> black(N), white(N);
    set<ll> leaves;

    auto dfs = [&](auto &&self, ll now, ll prev) -> void {
        if(prev != -1)
        {
            black[now] = white[prev];
            white[now] = black[prev] + white[prev];
        };
        bool leave = true;
        for(auto e : G[now]) if(e.to != prev) self(self, e.to, now), leave = false;
        if(leave) leaves.insert(now);
    };

    mint res1 = 1;
    black[0] = 1, white[0] = 0;
    dfs(dfs, 0, -1);
    for(auto leaf : leaves) res1 *= black[leaf] * white[leaf];

    mint res2 = 1;
    black = vector<mint>(N, 0);
    white = vector<mint>(N, 0);
    black[0] = 0, white[0] = 1;
    dfs(dfs, 0, -1);
    for(auto leaf : leaves) res2 *= black[leaf] * white[leaf];
