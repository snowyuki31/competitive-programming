...
int main()
{
    ...
    rep(i, N) PA[i] = Point(A[i].first, A[i].second), PB[i] = Point(B[i].first, B[i].second);

    vector<Point> CA = ConvexHull(PA);
    vector<Point> CB = ConvexHull(PB);
    ll L = len(CA);

    ld suma = 0, sumb = 0;
    rep(i, L) suma += abs(CA[(i + 1) % L] - CA[i]);
    rep(i, L) sumb += abs(CB[(i + 1) % L] - CB[i]);
