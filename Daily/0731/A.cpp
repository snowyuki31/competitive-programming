...
Point crosspoint(const Line& L1, const Line& L2){
    Point A = L1[0], B = L1[1];
    Point C = L2[0], D = L2[1];
    return A + (A - B) * (imag((A - C) * conj(C - D)) / imag((C - D) * conj(A - B)));
}
...
int main()
{
    ...
    rep(i, N) rep(j, N) if(i < j){
        Point cent = points[i] + points[j];
        cent /= 2;
        ls.pb(cent);
    }

    //三点の場合
    rep(i, N) rep(j, N) rep(k, N) if(i != j and j != k and k != i){
        auto a = points[i], b = points[j], c = points[k];
        if(!ccw(a, b, c)){
            ls.pb(crosspoint(bisector(Line(a, b)), bisector(Line(a, c))));
        }
    }
    ...
    for(auto s : ls){
        double tmp = 0;
        for(auto p : points) chmax(tmp, abs(s - p));
        chmin(res, tmp);
    }
