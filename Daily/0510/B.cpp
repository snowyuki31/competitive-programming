...
int main()
{
    ...
    vector<vector<ll>> state(H, vec(W, 0));

    auto memo = [&](auto&& self, ll i, ll j)->ll{

        if(i == H or j == W or grid[i][j] == '#') return 1;
        if(state[i][j] != 0) return state[i][j];

        ll res = -1;
        if(self(self, i + 1, j) == -1) res = 1;
        if(self(self, i, j + 1) == -1) res = 1;
        if(self(self, i + 1, j + 1) == -1) res = 1;
        return state[i][j] = res;
    };

    memo(memo, 0, 0);
    if(state[0][0] == 1) print("First");
    else print("Second");
