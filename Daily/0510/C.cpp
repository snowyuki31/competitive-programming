...
int main()
{
    ...
    rep(i, len(A)) if(A[i] != B[i]) a += A[i], b += B[i];
    ...
    rep(i, len(A)){
        if(A[i] != B[i]) a += A[i], b += B[i];
        if(A[i] == B[i] and ct <= 10) a += A[i], b += B[i], ++ct;
    }


    auto f = [&](auto &&self, string &s, ll ct = 0)->void {
        if(ct == 3){
            if(s == b) drop("YES");
            return;
        }

        rep(i, len(s)) rep(j, len(s)) if(i < j){
            string t = s;
            swap(t[i], t[j]);
            self(self, t, ct + 1);
        }
