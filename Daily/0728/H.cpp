...
int main()
{
    ll H, W;
    cin >> H >> W;
    Grid grid = getGrid(H, W);
    vec dx{0, 1, 0, -1}, dy{1, 0, -1, 0};

    P start;
    rep(i, H) rep(j, W) if(grid[i][j] == 's') start = {i, j};

    auto f = [&](double val) -> bool {

        queue<tuple<ll, ll, double, double>> que;
        vec2 visited(H, vec(W, 0));
        que.push({start.first, start.second, INF, 1});

        bool flag = false;
        while(!que.empty()){
            auto [i, j, res, modify] = que.front();
            que.pop();
            visited[i][j] = 1;
            if(grid[i][j] == 'g'){
                flag = true;
            ...
            rep(k, 4){
                ...
                double light = grid[nx][ny] - '0';
                light *= modify * double(99) / double(100);
                visited[nx][ny] = 1;
                if(light < val) continue;

                que.push({nx, ny, min(res, light), modify * double(99) / double(100)});
            }
        }
