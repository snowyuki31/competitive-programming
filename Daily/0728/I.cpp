...
int main()
{
    ...
    rep(i, N) rep(j, N) if(i != j){
        double x = (C[i] * X[i] + C[j] * X[j]) / double(C[i] + C[j]);
        double y = (C[i] * Y[i] + C[j] * Y[j]) / double(C[i] + C[j]);

        double resi = C[i] * max(x - X[i], y - Y[i]);
        double resj = C[j] * max(x - X[j], y - Y[j]);
        chmax(ans, max(resi, resj));
    }
