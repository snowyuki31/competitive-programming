...
int main()
{
    ...
    sort(ALL(A));
    vec pos, neg;
    for(auto a : A) (a >= 0 ? pos : neg).pb(a);

    ll mn = INF;
    ll sum = 0;
    for(auto a : A) sum += abs(a), chmin(mn, abs(a));
    
    if(A[0] < 0 and A.back() > 0){
        print(sum);
        rep(i, len(neg)) if(i != len(neg) - 1){
            cout << pos.back() << " " << neg[i] << '\n';
            pos.back() -= neg[i];
        }
        if(len(pos) == 1){
            cout << pos.back() << " " << neg.back() << '\n';
            return 0;
        }
        cout << neg.back() << " " << pos[0] << '\n';

        ll prev = neg.back() - pos[0];
        for (ll i = 1; i < len(pos) - 1; ++i){
            cout << prev << " " << pos[i] << '\n';
            prev -= pos[i];
        }
        cout << pos.back() << " " << prev << '\n';
        ...
        print(sum - mn * 2);
        if(A[0] < 0) sort(RALL(A));

        if(len(A) == 2){
            if(A.back() > 0) cout << A[1] << " " << A[0] << '\n';
            else cout << A[0] << " " << A[1] << '\n';
            return 0;
        }
        ...
        if(A.back() > 0) cout << A.back() << " " << prev << '\n';
        else cout << prev << " " << A.back() << '\n';
