...
int main()
{
    ...
    rep(i, N) (i % 2 == 0 ? even : odd)[V[i]]++;

    priority_queue<P> epq, opq;
    for(auto [key, val] : even) epq.push({val, key});
    for(auto [key, val] : odd) opq.push({val, key});

    epq.push({0, -1});
    opq.push({0, -1});

    priority_queue<P> tmp = opq;

    ll ans = INF;
    
    while(epq.top().second == tmp.top().second) tmp.pop();
    chmin(ans, N - epq.top().first - tmp.top().first);

    while(epq.top().second == opq.top().second) epq.pop();
    chmin(ans, N - epq.top().first - opq.top().first);
