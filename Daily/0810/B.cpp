...
int main()
{
    ...
    string S;
    cin >> S;
    const ull base = ull(1e9 + 7);
    vector<ull> power(N + 1);
    vector<ull> hash(N + 1);
    power[0] = 1;
    rep(i, N) power[i + 1] = power[i] * base;
    rep(i, N) hash[i + 1] = (hash[i] + S[i]) * base;
    ...
    auto f = [&](ll L) {
        ...
        for(ll i = L; i <= N; ++i){ 
            ull h = (hash[i] - hash[i - L] * power[L]);

            if(!mn.count(h)) mn[h] = i - L, mx[h] = i - L;
            else{ 
                chmin(mn[h], i - L);
                chmax(mx[h], i - L);
            }
        ...
        for(ll i = L; i <= N; ++i){ 
            ...
            if (a <= i - 2 * L or b >= i) return false;
        }

        return true;
    };

    auto res = bisearch(0, N, f);
