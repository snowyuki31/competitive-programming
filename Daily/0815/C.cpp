...
int main()
{
    ...
    vec3 dp(R + 1, vec2(G + 1, vec(B + 1, 0)));
    auto f = [&](auto&& self, ll x, ll y, ll z)->ll {
        if(dp[x][y][z]) return dp[x][y][z];
        if(x != R and y != G) chmax(dp[x][y][z], self(self, x + 1, y + 1, z) + r[x] * g[y]);
        if(y != G and z != B) chmax(dp[x][y][z], self(self, x, y + 1, z + 1) + g[y] * b[z]);
        if(z != B and x != R) chmax(dp[x][y][z], self(self, x + 1, y, z + 1) + b[z] * r[x]);
