...
struct StronglyConnectedComponents {
    ...
    StronglyConnectedComponents(Graph &g) : g(g), G(g.size()), R(g.size()), comp(g.size(), -1), used(g.size()){
        for(int i = 0; i < g.size(); ++i){ 
            for(auto& e : g[i]){ 
                G.add_arrow(e.from, e.to, e.weight);
                R.add_arrow(e.to, e.from, e.weight);
            }
        }
    }
    ...
    void rdfs(int idx, int cnt){
        ...
        comp[idx] = cnt;
        for (auto &e : R[idx]) rdfs(e.to, cnt);
    }
    ...
    void build(Graph &t) {
        ...
        int now = 0;
        for (auto i : order) if (comp[i] == -1) rdfs(i, now), ++now;
...
int main()
{
    ...
    auto isValid = [](P a, P b) {
        auto [ax, ay] = a;
        auto [bx, by] = b;
        return bx > ay or ax > by;
    };

    Graph G(4 * N);

    rep(i, N) G.add_arrow(2 * N + i, N + i), G.add_arrow(3 * N + i, i);

    rep(i, N) rep(j, N) if (i != j and !isValid(E[i], E[j])) G.add_arrow(2 * N + i, j), G.add_arrow(2 * N + j, i);
    rep(i, N) rep(j, N) if (i != j and !isValid(R[i], R[j])) G.add_arrow(3 * N + i, N + j), G.add_arrow(3 * N + j, N + i);

    rep(i, N) rep(j, N) if (i != j and !isValid(E[i], R[j])) G.add_arrow(2 * N + i, N + j), G.add_arrow(3 * N + j, i);
