...
int main()
{
    ...
    vector<ll> sz(N, 1);
    auto dfssz = [&](auto&& self, int now, int prev)->void {
        for(auto e : G[now]) if(e.to != prev) self(self, e.to, now);
        if(prev != -1) sz[prev] += sz[now];
    };
    dfssz(dfssz, 0, -1);

    vector<mint> dp(N, 0);
    rep(i, N) dp[i] = fac[sz[i] - 1];

    auto dfs1 = [&](auto&& self, int now, int prev)->void{
        for(auto e : G[now]) if(e.to != prev) self(self, e.to, now);
        if(prev != -1) dp[prev] *= dp[now] / fac[sz[now]];
    };
    dfs1(dfs1, 0, -1);

    
    print(dp);
    vector<mint> ans(N, fac[N - 1]);

    auto dfs2 = [&](auto&& self, int now, int prev, mint dpar, int dsz)->void{
        if(prev == -1) ans[now] = dp[now], dpar = 1; 
        else{
            ans[now] *= dpar / dsz;
            for(auto& e : G[now]) if(e.to != prev) ans[now] *= dp[e.to] / sz[e.to];

            dpar = dpar / dsz;
            for(auto& e : G[now]) dpar *= dp[e.to] / sz[e.to];
        }

        for(auto& e : G[now]) if(e.to != prev) self(self, e.to, now, dpar * sz[e.to] / dp[e.to], N - sz[e.to]);
    };
