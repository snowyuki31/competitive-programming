...
int main()
{
    ...
    SegmentTree<ll> mx(N, [](ll x, ll y) { return max(x, y); }, -INF);
    SegmentTree<ll> mn(N, [](ll x, ll y) { return min(x, y); }, INF);

    ll inv = 0;

    rep(i, N) mx.set(i, P[i]), mn.set(i, P[i]);

    rep(i, K) if(i >= 1) if(P[i - 1] > P[i]) ++inv;
    mx.build();
    mn.build();
    ...
    if(inv == 0) not_change.insert(0);
    else change.insert(0);

    for (ll i = K; i < N; ++i){
        ll prev = P[i - K];
        ll now = P[i];

        if(!(prev == mn.query(i - K, i + 1) and now == mx.query(i - K, i + 1))){
            change.insert(i - K + 1);
        }

        if(P[i - K] > P[i - K + 1]) --inv;
        if(P[i - 1] > P[i]) ++inv;

        if(inv == 0) not_change.insert(i - K + 1);
    }

    for(auto nc : not_change) change.erase(nc);

    print(len(change) + !not_change.empty());
