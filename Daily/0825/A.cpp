...
int main()
{
    ...
    vector<ll> ans(N);
    iota(ALL(ans), 0);
    rep(i, M) swap(ans[A[i]], ans[A[i] + 1]);
    // i -> ans[i] に移動

    //2^k回後にそれぞれどこにいるか
    vec2 dub(61, vec(N));
    dub[0] = ans; //1回後
    rep(i, N) dub[1][i] = ans[dub[0][i]]; //2回後
    for(ll k = 2; k < 61; ++k) rep(i, N) dub[k][i] = dub[k - 1][dub[k - 1][i]];

    vec res(N);
    iota(ALL(res), 0);

    rep(k, 61) if(D & (1LL << k)) rep(i, N) res[i] = dub[k][res[i]];

    vec out(N);
    rep(i, N) out[res[i]] = i + 1;
    for(auto o : out) print(o);
