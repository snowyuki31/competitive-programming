...
int main()
{
    ...
    ll ans = 0;
    rep(i, N) {
        auto [pos, dir] = rabbit[i];
        if(!Left.empty() and dir == 'R')
        {
            deque<ll> RR = Right, LL = Left;
            //左に寄せる場合
            ll tmpr = 0;
            ll posr = RR.empty() ? 0 : RR.back();
            ll posl = RR.empty() ? 1 : RR.back() + 1;
            while(!RR.empty()) tmpr += abs(posr - RR.back()), --posr, RR.pop_back();
            while(!LL.empty()) tmpr += abs(posl - LL.front()), ++posl, LL.pop_front();
            ...
            posr = LL.front() - 1;
            posl = LL.front();
            while(!RR.empty()) tmpl += abs(posr - RR.back()), --posr, RR.pop_back();
            while(!LL.empty()) tmpl += abs(posl - LL.front()), ++posl, LL.pop_front();

            ans += max(tmpr, tmpl);
            Right.clear(), Left.clear();
        }

        if(dir == 'R') Right.push_back(pos);
    ...
    if(!Left.empty() or !Right.empty())
    {
        ...
        //右に寄せる場合
        ll tmpl = 0;
        posr = LL.empty() ? L : LL.front() - 1;
        posl = LL.empty() ? L + 1 : LL.front();
        while(!RR.empty()) tmpl += abs(posr - RR.back()), --posr, RR.pop_back();
        while(!LL.empty()) tmpl += abs(posl - LL.front()), ++posl, LL.pop_front();
