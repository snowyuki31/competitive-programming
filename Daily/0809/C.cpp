...
ll solve(ll N, Grid& grid){
    //x行目選ぶ
    ll res = INF, row = 0, tar = 0;
    rep(x, N){
         rep(j, N) if(grid[x][j] == '#'){
             ll ct = 0;
            rep(k, N) ct += grid[j][k] == '.';
            if(chmin(res, ct)) row = x, tar = j;
        }
    } 

   ll ans = 0;
    string replacer = grid[row];
    rep(j, N) if(grid[tar][j] == '.'){
        rep(i, N) grid[i][j] = replacer[i];
        ++ans;
    }

    rep(j, N){
        bool flag = false;
        rep(i, N) if(grid[i][j] == '.') flag = true;
        ans += flag;
    }
    return ans;
}
...
int main()
{
    ...
    Grid grid = getGrid(N, N);

    bool flag = false;
    for (auto g : grid) for (auto c : g) if (c == '#') flag = true;
    if (!flag) drop(-1);

    Grid rev = grid;
    rep(i, N) rep(j, N) rev[j][i] = grid[i][j];

    print(min({solve(N,  grid), solve(N, rev)}));
