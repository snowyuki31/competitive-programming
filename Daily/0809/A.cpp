...
int main()
{
    ...
    vec rdist = R.dijkstra(N - 1);

    vector<ll> possible(N);
    rep(i, N) possible[i] = (dist[i] != INF and rdist[i] != INF);

    G = Graph(N);
    for(auto& [a, b, c] : E){ 
        if(possible[a] and possible[b]){
            if (a == b) {
                if(c > 0) drop(-1);
            }else{
                G.add_arrow(a, b, -c);
    ...
    vec res = G.bellman_ford(0);

    if(res.empty()) drop(-1);

    print(res.back() > 0 ? 0 : abs(res.back()));
