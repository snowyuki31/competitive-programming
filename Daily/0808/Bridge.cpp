...
// add : 見ている根に対する処理
//
// 例 : 最も遠い点についてのクエリの場合、mergeはmax関数、addは重みを渡す
struct Rerooting {
    /* メモ化する値の定義 */
    struct MEMO {
        long long val;
        MEMO(long long val) : val(val) {}
    };
    const MEMO IE = MEMO(-1); //単位元
    function<MEMO(MEMO, MEMO)> merge = [](MEMO cum, MEMO m) -> MEMO{
        return MEMO(max(cum.val, m.val));
    };
    function<MEMO(MEMO)> add = [](MEMO m)-> MEMO{
        return MEMO(m.val + 1);
    };
    ...
    Rerooting(Graph G) : G(G), memo(G.size()), ans(G.size(), IE) {}
    ...
    void test() {
        ...
        for(int i = 0; i < G.size() ; ++i){
            memo[i] = vector<MEMO>(G[i].size(), IE);
            cum = merge(cum, memo[i][0]);
        }
    }

     vector<ll> result(){
        vector<ll> res;
        for(auto a : ans) res.emplace_back(a.val);
        return res;
    }

    MEMO dfs (int v, int p = -1){
        MEMO cum = IE;
        int sz = G[v].size();
        memo[v] = vector<MEMO>(G[v].size(), IE);
        for(int i = 0; i < sz; ++i){
            auto e = G[v][i];
            if(e.to != p){
                memo[v][i] = dfs(e.to, v);
                cum = merge(cum, memo[v][i]);
            }
        }
        return add(cum);
    }

    void bfs(int v, const MEMO& m, int p = -1){
        ll sz = G[v].size(), res = 0;
        for(int i = 0; i < sz; ++i){
            if(G[v][i].to == p) memo[v][i] = m, res = G[v][i].weight;
        }
        vector<MEMO> memo_l(sz + 1, IE), memo_r(sz + 1, IE);
        for(int i = 0; i < sz; ++i)  memo_l[i + 1] = merge(memo_l[i], memo[v][i]);
        for(int i = sz - 1; i >= 0;  --i) memo_r[i] = merge(memo_r[i + 1], memo[v][i]);

        ans[v] = add(memo_l[sz]);

        for(int i = 0; i < sz; ++i){
            auto e = G[v][i];
            if(e.to != p) bfs(e.to, add(merge(memo_l[i], memo_r[i + 1])), v);
        }
    }
};
