...
int main()
{
    ...
    vec2 CUM(H + 1, vec(W + 1, 0));

    rep1(i, H) rep1(j, W) CUM[i][j] = CUM[i - 1][j] + C[i - 1][j - 1];
    rep1(i, H) rep1(j, W) CUM[i][j] += CUM[i][j - 1];

    ll ans = 0;
    rep(x, H + 1) rep(y, W + 1) rep(xx, H + 1) rep(yy, W + 1){
        if(CUM[xx][yy] - CUM[x][yy] - CUM[xx][y] + CUM[x][y] == 0){
            chmax(ans, (yy - y) * (xx - x));
        }
    }
    print(ans);
