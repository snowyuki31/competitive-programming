...
int main()
{
    ...
    map<string, ll> mp;
    ll ct = 0;
    for(auto& s : st) if(!mp.count(s)) mp[s] = ct++;

    vector<string> index(ct);
    for(auto& [key, val] : mp) index[val] = key;
    ...
    for(auto& s : st){
        for(auto& g : st){
            ll tmp = 0;
            rep(i, len(s)) tmp += s[i] != g[i];

            if(tmp == 1){
                add_edge(G, mp[s], mp[g]);
            }
        }
    }
    ...
    vec path = shortest_path(G, mp[first], mp[last]);

    if(path.front() != mp[first]) drop(-1);

    print(max(len(path) - 2, 0LL));
    for(auto p : path){
        print(index[p]);
    }
