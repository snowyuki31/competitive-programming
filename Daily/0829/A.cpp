...
int main()
{
    ...
    ll need = 0;
    rep(y, H) rep(x, W) need += grid[y][x] != ((y * W + x + 1) % (H * W));

    auto dfs = [&](auto &self, P now, P prev, int ct)->void {
        auto [i, j] = now;
        auto [pi, pj] = prev;

        if(need > (24 - ct) * 2) return;
        if(need == 0){
            chmin(ans, ct); return;
        }
        if(ct >= 23) return;
        ...
        rep(p, 4){
            ...
            ll bef = (grid[i][j] != ((i * W + j + 1) % (H * W))) + (grid[ni][nj] != ((ni * W + nj + 1) % (H * W)));
            swap(grid[i][j], grid[ni][nj]);
            ll aft = (grid[i][j] != ((i * W + j + 1) % (H * W))) + (grid[ni][nj] != ((ni * W + nj + 1) % (H * W)));
            ll cd = aft - bef;
            need += cd;
            self(self, {ni, nj}, {i, j}, ct + 1);
            swap(grid[i][j], grid[ni][nj]);
            need -= cd;
        }
    };
    ...
    P s;
    rep(i, H) rep(j, W) if(!grid[i][j]) s = {i, j};
    dfs(dfs, s, {-1, -1}, 0);
    print(ans);
