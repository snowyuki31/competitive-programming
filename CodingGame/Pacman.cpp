...
struct pacman {
    ...
    pacman() : id(0), x(0), y(0), type(""), speed(0), ability(0), prevx(-1), prevy(-1), isRandom(false), movingx(-1), movingy(-1), moved(false) {}
...
bool move_randomly(pacman& m, vector<P>& cand, set<P>& ls){
    if(m.moved) return false;
...
bool buttle(pacman& m, vector<pacman>& enemy, vector<string>& grid){
    if(m.moved) return false;
    int ct = 0;
    for(auto& e : enemy) ct += !!isClose(m, e, grid, 4);
    
    for(auto& e : enemy){
        int dist;
        if(m.speed != 0 and e.speed != 0) dist = 4;
        else if(m.speed != 0) dist = 3; //自分がスピード
        else if(e.speed != 0) dist = 3; //相手がスピード
        else dist = 2;

        if(isClose(m, e, grid, 4) and ct >= 2) return escape(m, enemy, grid); 
...
P beamsearch(pacman& m, vector<vector<P>>& pellet, set<P>& pacpos, vector<string>& grid, int depth, int turn){
    //BFSでpelletを評価していく
    //探索して、最良のルートを取る
    priority_queue<pair<double, P>> pq;
    queue<tuple<P, double, int>> que;
    set<P> visit;

    pq.push({0, {-1, -1}});
    que.push({{m.y, m.x}, 0, 0});
    visit.insert({m.y, m.x});
    
    while(!que.empty()){
        auto [now, val, d] = que.front();
        que.pop();

        if(d == depth){
            pq.push({double(val) / double(d), now});
            continue;
        }
        ...
        rep(i, 4){
            ...
            if(pacpos.count({nexty, nextx})) val -= 20000;
            double score = double(eval) / double(turn - decay + 1);

            que.push({{nexty, nextx}, val + score, d + 1});
            pq.push({double(val + score) / double(d + 1), {nexty, nextx}});
            visit.insert({nexty, nextx});
        }
    }
    if(pq.top().first <= 10) return {-1, -1};

    cerr << "score : " << pq.top().first << endl;
    return pq.top().second;
}

bool search_move(pacman& m, vector<vector<P>>& pellet, set<P>& pacpos, vector<string>& grid, int depth, int turn){
    if(m.moved) return false;
    auto [y, x] = beamsearch(m, pellet, pacpos, grid, depth, turn);
    if(x == -1 or y == -1) return false;
    return move(m, y, x);
}
...
int main(){
    ...
    for(int y = 0; y < height; ++y) for(int x = 0; x < width; ++x) if (isOkay(grid, y, x)){
        ...
        pellet[y][x] = {1000, 0}; //最初は1あるとして初期化
    }
    ...
    while (1){
        ...
        for (int i = 0; i < visiblePelletCount; i++){
            ...
            //見た情報は高い確信度
            if(value == 1) pellet[y][x] = {1000, turn};
            else pellet[y][x] = {50000, turn}; //5マス先までは10倍以上の価値を持たせる
        }
        ...
        */
        ...
        for(auto& m : survive){
            //敵が近くにいれば戦うか考える
            if(buttle(m, enemy, grid)) cerr << "pacman " << m.id << " buttled!" << endl;
            ...
            //それ以外は探索
            if(search_move(m, pellet, pacpos, grid, 10, turn)) cerr << "pacman " << m.id << " move with search" << endl;

            //残りの探索
            if(move_randomly(m, cand, unchecked)) cerr << "Pacman " << m.id << " searching unchecked zone" << endl;

            //候補がない場合
            if(move_randomly(m, unvis, unvisited)) cerr << "Pacman " << m.id << " searching unvisited zone" << endl;
        }
        cerr << endl;
        cout << endl;
