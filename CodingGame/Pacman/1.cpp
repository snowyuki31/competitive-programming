...
const int c_super_pellet = 5000 * 100 * 100;
...
P beamsearch(pacman& m, vector<vector<P>>& pellet, set<P>& pacpos, vector<string>& grid, int depth, int turn, int unvisited){
    //BFSでpelletを評価していく
    //探索して、最良のルートを取る
    ...
    while(!que.empty()){
        ...
        rep(i, 4){
            ...
            if(pacpos.count({nexty, nextx})) val -= c_pellet * 10;
            double score;
            if(eval == uc_pellet){
                eval *= (unvisited / 100); //未知のゾーンはターン経過とともに価値減
                score = double(eval);
            }else{
                score = double(eval) / pow(double(turn - decay + 1), 2);
            }
    ...
    if(pq.top().first <= 200) return {-1, -1};

    cerr << "score : " << pq.top().first << endl;
    return pq.top().second;
}

bool search_move(pacman& m, vector<vector<P>>& pellet, set<P>& pacpos, vector<string>& grid, int depth, int turn, int unvisited){
    if(m.moved) return false;
    auto [y, x] = beamsearch(m, pellet, pacpos, grid, depth, turn, unvisited);
    if(x == -1 or y == -1) return false;
    return move(m, y, x);
}
    ...
    while (1){
        ...
        */
        ...
        for(auto& m : survive){
            ...
            //それ以外は探索
            if(search_move(m, pellet, pacpos, grid, 10, turn, unvisited.size())) cerr << "pacman " << m.id << " move with search" << endl;
