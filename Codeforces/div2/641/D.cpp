...
int main()
{
    ...
    while(t--){
        ...
        ll idx = ls[(len(ls) + 1) / 2 - 1];

        vec f, b;
        rep(i, idx - 1) f.pb(A[i]);
        for (ll i = idx + 1; i < N; ++i) b.pb(A[i]);
        sort(ALL(f)), sort(ALL(b));

        if(f.empty() and b.empty()) print("yes");
        else if(f.empty()){
            ll fb = b[(len(b) + 1) / 2 - 1];
            vec res{fb, A[idx]};
            sort(ALL(res));
            if(res[0] == K) print("yes");
            else print("no");
        }else if(b.empty()){
            ll fx = f[(len(f) + 1) / 2 - 1];
            vec res{fx, A[idx]};
