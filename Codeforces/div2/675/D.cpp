...
int main()
{
    ...
    Graph G(M + 2);
    //Sからワープ先までの辺
    G.add_edge(0, M + 1, abs(fx - sx) + abs(fy - sy));
    rep(i, M){
        auto [x, y] = X[i];
        ll d = min(abs(x - sx), abs(y - sy));
        G.add_edge(0, index[X[i]] + 1, d);
    }
    ...
    rep(i, M){
        auto [x, y] = X[i];
        ll d = abs(x - fx) + abs(y - fy);
        G.add_edge(index[X[i]] + 1, M + 1, d);
    }
    ...
    rep(i, M - 1){
        auto [x, y] = X[i];
        auto [x_, y_] = X[i + 1];
        ll d = min(abs(x - x_), abs(y - y_));
        G.add_edge(index[X[i]] + 1, index[X[i + 1]] + 1, d);
    }
    sort(ALL(X), [&](P f, P s) { return f.second < s.second; });
    rep(i, M - 1){
        auto [x, y] = X[i];
        auto [x_, y_] = X[i + 1];
        ll d = min(abs(x - x_), abs(y - y_));
