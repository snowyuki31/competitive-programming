...
template< typename T >
pair<vector<T>, vector<T>> dijkstra(Graph &g, T s, bool restore = false){
    vector<T> dist(g.size(), INF);
    vector<T> cnt(g.size(), 0);
    ...
    while(!que.empty()){
        ...
        for(auto &e : g[idx]){
            ...
            if(dist[e.to] == next_cost) cnt[e.to] += cnt[e.from];
...
int main()
{
    ...
    auto [dist, cnt] = dijkstra(R, p.back());

    ll mn = 0, mx = 0;
    for(ll i = K - 1; i > 0; --i){
        ll d = dist[p[i - 1]] - dist[p[i]];

        if(d != 1) ++mn, ++mx;
        else{
            ll ct = 0;
            for(auto e : G[p[i - 1]]){
                if(e.to != p[i] and dist[e.from] - dist[e.to] == 1) ++ct;
            }
            if(ct >= 1) ++mx;
        }
    }
