...
int main()
{
    ...
    cin >> n >> x;
    vector<ll> d(3 * n);
    rep(i, n) cin >> d[i], d[i + n] = d[i], d[i + 2 * n] = d[i];

    SegmentTree<ll> segtree(3 * n, [](ll x, ll y) { return x + y; }, 0);
    SegmentTree<ll> sumtree(3 * n, [](ll x, ll y) { return x + y; }, 0);
    rep(i, 3 * n) segtree.set(i, (1 + d[i]) * d[i] / 2), sumtree.set(i, d[i]);
    segtree.build();
    sumtree.build();
    ...
    rep(l, n){
        while(sum < x and r <= 2 * n){
            sum += d[r];
            ++r;
        }

        ll res = segtree.query(l + 1, r - 1);

        ll tp_num = sumtree.query(l + 1, r - 1);
        ll num = x - tp_num;

        if(num <= d[l]){
            res += (d[l] + d[l] - num + 1) * num / 2;
        }else if(num <= d[r - 1]){
            res += (1 + num) * num / 2;
        }else{
            res += (1 + d[r - 1]) * d[r - 1] / 2;
            num -= d[r - 1];
            res += (d[l] - num + 1 + d[l]) * num / 2;
        }

        //cout << l << " " << r << " " << sum << " " << res << endl;
