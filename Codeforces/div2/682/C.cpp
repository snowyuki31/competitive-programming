...
int main()
{
    ...
    while(T--){
        ...
        rep(i, N) rep(j, M){
            ...
            if(i >= 1 and A[i - 1][j] == A[i][j] + 1) continue;
            if(j >= 1 and A[i][j - 1] == A[i][j] + 1) continue;
            if(i + 1 < N and A[i + 1][j] == A[i][j] + 1) continue;
            if(j + 1 < M and A[i][j + 1] == A[i][j] + 1) continue;

            ++A[i][j];
        }
        for(auto a : A) print(a);
