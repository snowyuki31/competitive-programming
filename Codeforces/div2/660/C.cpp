...
int main()
{
    ...
    while (T--){
        ...
        auto dfs = [&](auto &&self, ll now, ll prev)->void {
            for(auto e : G[now]) if(e.to != prev){
                self(self, e.to, now);
            }

            if(prev != -1) passed[prev] += passed[now] + People[now];

            ll gg = People[now] + H[now] + passed[now];
            ll bb = People[now] - H[now] + passed[now];

            if (gg % 2 == 1 or bb % 2 == 1 or gg < 0 or bb < 0) flag = false;
            gg /= 2, bb /= 2;
            ll exist = People[now] + passed[now];

            if(gg > exist or bb > exist) flag = false;

            good[now] = gg, bad[now] = bb;
        };

        auto dfs2 = [&](auto &self, ll now, ll prev) -> void {
            for(auto e : G[now]) if(e.to != prev){
                self(self, e.to, now);
            }
            ...
            if(bad[now] - People[now] > sum) flag = false; 
        };
