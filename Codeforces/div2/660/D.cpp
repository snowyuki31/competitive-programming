...
int main()
{
    ...
    Graph G(N);
    rep(i, N) if(B[i] != -1) add_edge(G, i, B[i] - 1);

    vec start;
    rep(i, N) if (B[i] == -1) start.pb(i);

    ll ans = 0;
    auto dfs = [&](auto &&self, ll now, ll prev, ll ct = 1)->void {
        if(A[now] < 0) ans += ct * A[now];
        else ans += ct * ct * A[now];
        
        for(auto e : G[now]) if(e.to != prev){
            self(self, e.to, now, ct + 1);
        }
    };
