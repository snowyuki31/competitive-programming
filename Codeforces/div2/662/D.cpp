...
int main()
{
    ...
    ull t1 = 1;
    ull def = 0;
    vector<vector<ull>> hash(N, vector<ull>(M));
    ...
    rep(i, N) rep(j, M) hash[i][j] = mp[grid[i][j]];

    vector<vector<ull>> cum(N + 1, vector<ull>(M + 1));
    rep1(i, N) rep1(j, M) cum[i][j] = hash[i - 1][j - 1];
    rep1(i, N) rep1(j, M) cum[i][j] += cum[i - 1][j];
    rep1(i, N) rep1(j, M) cum[i][j] += cum[i][j - 1];

    set<ull> st;

    ll ct = 0;
    rep1(i, N) rep1(j, M){
        rep(L, min({ i, j, N + 1 - i, M + 1 - j })) {
            ull sum = 0;
            if (L > 0) sum = cum[i + L - 1][j + L - 1] - cum[i + L - 1][j - L] - cum[i - L][j + L - 1] + cum[i - L][j - L];
            else sum = mp[grid[i - 1][j - 1]];
            if(L > 0) sum += hash[i + L - 1][j - 1] + hash[i - 1][j + L - 1] + hash[i - L - 1][j - 1] + hash[i - 1][j - L - 1];
            if (sum % mp[grid[i - 1][j - 1]] == 0){ 
                ll rem = sum / mp[grid[i - 1][j - 1]];
                if (rem == 2 * L * (L + 1) + 1) ++ct;
            }
        }
    }
