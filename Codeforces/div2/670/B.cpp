...
long long solve(){
    ...
    vector<int> m, nm;
    for(auto &a : A) (a < 0 ? m : nm).emplace_back(a);
    sort(ALL(m));
    sort(ALL(nm));

    long long ans = -INF;
    for (int x = 0; x <= 5; ++x){
        if(m.size() < x or nm.size() < 5 - x) continue;
        long long res = 1;
        if(x % 2){
            for(int i = 0; i < x; ++i) res *= m[m.size() - 1 - i];
            for(int i = 0; i < 5 - x; ++i) res *= nm[i];
        }
        else{
            for(int i = 0; i < x; ++i) res *= m[i];
            for(int i = 0; i < 5 - x; ++i) res *= nm[nm.size() - 1 - i];
        }
        chmax(ans, res);
    }
    return ans;
