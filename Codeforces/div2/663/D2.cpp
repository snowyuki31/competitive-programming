...
int main()
{
    ...
    rep(i, N){
        string S;
        cin >> S;
        rep(j, M) grid[i][j] = S[j] == '1';
    }

    if (N == 1 or M == 1) drop(0);
    if (N >= 4 and M >= 4) drop(-1);
    if (N == 2){ 
        vec res(M - 1);
        rep(y, M - 1) rep(i, 2) rep(j, 2) res[y] += grid[i][y + j], res[y] %= 2;
        ll ct = 0;
        rep(y, M - 1){
            if(res[y]) continue;
            ++ct;
            if(y + 1 < M - 1) if(res[y + 1] == 0) res[y + 1] = !res[y + 1]; 
        }
        drop(ct);
    }
    if (M == 2){ 
        vec res(N - 1);
        rep(y, N - 1) rep(i, 2) rep(j, 2) res[y] += grid[y + i][j], res[y] %= 2;
        ll ct = 0;
        rep(y, N - 1) {
            if (res[y]) continue;
            ++ct;
            if (y + 1 < N - 1) if (res[y + 1] == 0) res[y + 1] = !res[y + 1];
        }
        drop(ct);
    }


    vec2 res(N - 1, vec(M - 1));
    rep(x, N - 1) rep(y, M - 1){
        rep(i, 2) rep(j, 2){
            res[x][y] += grid[x + i][y + j];
        }
        res[x][y] %= 2;
    }
