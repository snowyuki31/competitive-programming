...
int main()
{
    ...
    if(N < M){ 
        vec2 tmp(M, vec(N));
        rep(i, N) rep(j, M) tmp[j][i] = grid[i][j];
        grid = tmp;
        swap(N, M);
    }

    
    vec2 dp(N + 1, vec(1 << M + 1, INF));
    rep(i, 1 << M) {
    ...
    rep1(i, N - 1){
        rep(j, 1 << M){
            rep(k, 1 << M){
                ...
                if(sum1 and sum2) chmin(dp[i][k], dp[i - 1][j] + ct);
            }
        }
    }

    ll ans = INF;
    rep(j, 1 << M) chmin(ans, dp[N - 1][j]);
    print(ans);
