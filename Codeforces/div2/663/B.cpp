...
int main()
{
    ...
    while(T--){ 
        ...
        UnionFind UF(N * M + 10);
        rep(i, N) rep(j, M){
            if(grid[i][j] == 'R') if(j + 1 < M) UF.unite(i * M + j, i * M + j + 1);
            if(grid[i][j] == 'L') if(j - 1 >= 0) UF.unite(i * M + j, i * M + j - 1);
            if(grid[i][j] == 'U') if(i - 1 >= 0) UF.unite(i * M + j, (i - 1) * M + j);
            if(grid[i][j] == 'D') if(i + 1 < N) UF.unite(i * M + j, (i + 1) * M + j);
        }

        set<ll> st;
        rep(i, N) rep(j, M){
            st.insert(UF.root(i * M + j));
        }
        print(len(st) - 1);
