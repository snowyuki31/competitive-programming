...
int main()
{
    ...
    while(T--){ 
        ...
        rep(i, N) {
            char &c1 = S[i], &c2 = S[g(i + 1)], &c3 = S[g(i + 2)], &c4 = S[g(i + 3)];
            if(c1 == 'R' and c2 == 'L' and c3 == 'L') c1 = c2 = c3 = 'X';
            if(c1 == 'R' and c2 == 'R' and c3 == 'L') c1 = c2 = c3 = 'X';
        }

        rep(i, N) {
            char &c1 = S[i], &c2 = S[g(i + 1)], &c3 = S[g(i + 2)], &c4 = S[g(i + 3)];
            if(c1 == 'R' and c2 == 'L') c1 = c2 = 'X';
        }

        deque<char> deq(ALL(S));

        ll ans = 0;
        while(!deq.empty())
        {
            if(deq.front() != 'L' and deq.front() != 'R'){
                deq.pop_front();
                continue;
            }
            ll ct = 0;
            while(!deq.empty()){
                if (deq.front() == 'L' or deq.front() == 'R') ++ct, deq.pop_front();
                else break;
            }
            while(!deq.empty()){
                if (deq.back() == 'L' or deq.back() == 'R') ++ct, deq.pop_back();
                else break;
            }
            ans += devc(ct, 3);
        }
