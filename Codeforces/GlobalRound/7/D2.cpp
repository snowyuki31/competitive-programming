...
int main()
{
    ...
    while (T--){
        ...
        string X = "$";
        for (ll i = idx; i <= N - 1 - idx; ++i) X += S[i], X += "$";
        vector<int> rad = manacher(X);
        ...
        string res = "";
        for (ll i = 0; i < 2 * tmp; ++i) if(X[i] != '$') res += X[i];
        ...
        for (ll i = 0; i < 2 * tmp2; ++i) if(X[i] != '$') res2 += X[i];

        string rans = ans;
        reverse(ALL(rans));

        string out1 = ans + res + rans;
        string out2 = ans + res2 + rans;

        //print(ans);
        print(len(out1) > len(out2) ? out1 : out2);
    }
