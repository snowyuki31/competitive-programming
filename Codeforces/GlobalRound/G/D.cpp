...
int main()
{
    ...
    //x座標に対する最大高さ
    vec PX(1e6 + 10, -1);
    for(auto [x, y] : torch) PX[x] = y;
    for(ll i = 1e6 + 8; i >= 0; --i) chmax(PX[i], PX[i + 1]);

    //変化点のリスト
    vec ls{0};
    for(ll i = 1; i <= 1e6 + 9; ++i) if(PX[i] != PX[ls.back()]) ls.pb(i);
    sort(ALL(rob));

    //位置xに対する次の変化点
    vec NEXT(1e6 + 15, INF);
    for(auto l : ls) NEXT[l] = l;
    for(ll i = 1e6 + 9; i >= 0; --i) chmin(NEXT[i], NEXT[i + 1]);
    for(auto l : ls) NEXT[l] = NEXT[l + 1];


    //初期値
    ll mn = 0;
    for(auto &[x, y] : rob) chmax(mn, PX[x] - y + 1);
    ...
    while(1){
        ...
        if(rob.front().first > 1e6 + 5) break;
    }
