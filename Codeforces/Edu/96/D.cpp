...
struct RunLengthEncoding{
    ...
    RunLengthEncoding(string &S) : S(S) {
        char tar;
        int ct = 0;
        for (auto &c : S){
            if (!ct) tar = c, ++ct;
                ...
                if (tar == c) ++ct;
                else ch.emplace_back(tar), num.emplace_back(ct), tar = c, ct = 1;
            }
        }
        if(ct) ch.emplace_back(tar), num.emplace_back(ct);
    }
...
int main()
{
    ...
    while(T--){ 
        ...
        RunLengthEncoding RLE(S);

        deque<P> room;
        //余剰数
        rep(i, len(RLE)){
            auto [ch, num] = RLE[i];
            if(num - 1) room.push_back({i, num - 1});
        }

        ll ans = 0;
        rep(i, len(RLE)){
            while(!room.empty()){
                auto [idx, num] = room.front();
                if(idx < i) room.pop_front();
                else break;
            }
