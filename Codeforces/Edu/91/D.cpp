...
int main()
{
    ...
    ll ans = 0;
    rep(i, len(index) - 1){
        ll num = index[i + 1] - index[i] - 1;

        if(num == 0) continue;

        ll left, right;
        if(index[i] == -1) left = -INF;
        else left = A[index[i]];

        if(index[i + 1] == N) right = -INF;
        else right = A[index[i + 1]];

        ll ct = 0; //番兵で消せる個数
        for (ll j = index[i] + 1; j <= index[j + 1] - 1; ++j){
            ct += A[j] <= max(left, right);
        }

        if(!ct){
            //番兵で1個も消せない場合, Bスペルだけで0に出来ない
            if(num < K) drop(-1);
