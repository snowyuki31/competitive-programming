...
int main()
{
    ...
    while (T--){
        ...
        map<char, set<ll>> mp;
        rep(i, len(S)) mp[S[i]].insert(i);
        ll ans = 0;

        //長さ奇数の場合
        for(auto [k, v] : mp) chmax(ans, len(v));

        //長さ偶数の場合
        for (char x = '0'; x <= '9'; ++x){
            for (char y = '0'; y <= '9'; ++y) if(x != y){
                if(!mp.count(x) or !mp.count(y)) continue;
                ll res = 1;
                ll now = *mp[x].begin();


                while(1){
                    auto itr = mp[y].upper_bound(now);
                    if(itr == mp[y].end()) break;
                    now = *itr;
                    ++res;

                    auto itr2 = mp[x].upper_bound(now);
                    if(itr2 == mp[x].end()) break;
                    now = *itr2;
                    ++res;
                }

                if(res % 2 == 0) chmax(ans, res);
            }
        }
