...
int main()
{
    ...
    SegmentTree<ll> segtree(n, [](ll x, ll y) { return x + y; }, 0);
    SegmentTree<ll> st(n, [](ll x, ll y) { return x + y; }, 0);

    rep(i, n) segtree.set(i, a[i]);
    rep(i, n) st.set(p[i] - 1, a[i]);
    ...
    rep(i, n - 1){
        ...
        ll over = segtree.query(0, i + 1);
        ll under = st.query(0, i + 1);
        chmin(ans, over + under);
    }
    print(ans);
