...
map<ll,ll> Prime_Factorization(ll N){
    map<ll,ll> PF;
    for(ll i = 2; i * i <= N; i++) while(!(N%i)) PF[i]++, N /= i;
    if(N != 1) PF[N] = 1;
    return PF;
}
...
int main()
{
    ...
    rep(i, N){
        ll a;
        cin >> a;
        map<ll, ll> pm = Prime_Factorization(a);
        vec plist;
        for(auto [key, val] : pm) plist.push_back(key);
        ll L = len(plist);
        if(len(pm) == 1){
            d1.push_back(-1);
            d2.push_back(-1);
            ...
            bool flag = true;
            rep(i, L) rep(j, L) if(flag){
                if(gcd(a, plist[i] + plist[j]) == 1){
                    d1.push_back(plist[i]);
                    d2.push_back(plist[j]);
                    flag = false;
                }
            }
            if(flag){
