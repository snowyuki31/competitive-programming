...
int main()
{
    ...
    vector<priority_queue<ll>> pq(3);
    pq[0] = priority_queue<ll>(ALL(r));
    pq[1] = priority_queue<ll>(ALL(g));
    pq[2] = priority_queue<ll>(ALL(b));

    ll ans = 0;
    while(1)
    {
        ll ct = 0;
        rep(i, 3) ct += pq[i].empty();
        if(ct >= 2) break;

        bool operated = false;
        rep(i, 3) if(!operated) if(pq[i].empty())
        {
            ans += pq[(i + 1) % 3].top() * pq[(i + 2) % 3].top();
            pq[(i + 1) % 3].pop(), pq[(i + 2) % 3].pop();
            operated = true;
        }
        if(operated) continue;

        vector<P> X{{pq[0].top(), 0}, {pq[1].top(), 1}, {pq[2].top(), 2}};
        sort(RALL(X));
