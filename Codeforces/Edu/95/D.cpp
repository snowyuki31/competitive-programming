...
int main()
{
    ...
    //クエリ先読みで使われる座標を拾う
    set<ll> used(ALL(p));
    for(auto [c, q] : query) used.insert(q);

    //座標圧縮
    ll L = len(used);
    vec get_val(ALL(used));
    unordered_map<ll, ll> get_idx;
    rep(i, L) get_idx[get_val[i]] = i;

    //BIT二本
    BinaryIndexedTree<ll> exist(L);
    BinaryIndexedTree<ll> sum(L);
    ...
    auto f = [&](ll x) {
        ...
        auto gl = [&](ll y) {
            //[0, y - 1]
            ll suml = sum.sum(0, y - 1);
            ll ctl = exist.sum(0, y - 1);
            //[y, x - 1]
            ll sumr = sum.sum(y, x - 1);
            ll ctr = exist.sum(y, x - 1);

            return abs(ctl * get_val[y] - suml) + abs(sumr - ctr * get_val[y]);
        };
        ll mnl = 0;
        if(x > 0) mnl = gl(ternarySearch(0, x, gl));

        //右側[x, L - 1]三分探索
        auto gr = [&](ll y) {
            //[x, y - 1]
            ll suml = sum.sum(x, y - 1);
            ll ctl = exist.sum(x, y - 1);
            //[y, L - 1]
            ll sumr = sum.sum(y, L - 1);
            ll ctr = exist.sum(y, L - 1);

            return abs(ctl * get_val[y] - suml) + abs(sumr - ctr * get_val[y]);
        };

        ll mnr = 0;
        if(x < L) mnr = gr(ternarySearch(x, L, gr));

        return mnl + mnr;
    };

    ll mn = f(ternarySearch(0, L, f));
    print(mn);

    for(auto [t, x] : query){
        if(!t){
            exist.add(get_idx[x], -1);
            sum.add(get_idx[x], -x);

            print(f(ternarySearch(0, L, f)));
        }
        else{
            exist.add(get_idx[x], 1);
            sum.add(get_idx[x], x);
