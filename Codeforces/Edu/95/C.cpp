...
int main()
{
    ...
    while(T--){ 
        ...
        rep(i, N){
            //friend 1, you 1
            if(i + 2 <= N) G.add_arrow(i, i + 2, A[i]);
            //friend 1, you 2
            if(i + 3 <= N) G.add_arrow(i, i + 3, A[i]);
            //friend 2, you 1
            if(i + 3 <= N) G.add_arrow(i, i + 3, A[i] + A[i + 1]);
            //friend 2, you 2
            if(i + 4 <= N) G.add_arrow(i, i + 4, A[i] + A[i + 1]);
        }
        G.add_arrow(N - 1, N, A[N - 1]);
        if(N >= 2) G.add_arrow(N - 2, N, A[N - 2] + A[N - 1]);

        vec dist = G.dijkstra(0);
        print(dist[N]);
    }
