...
int main()
{
    ...
    while (T--){
        ...
        auto dfs = [&](auto& self, ll now, ll prev)->void {
            bool leave = true;
            for (auto e : G[now]) if (e.to != prev) self(self, e.to, now), leave = false;
        ...
        using wedge = tuple<ll, ll, ll>;

        vector<wedge> modified;
        for(auto [p, w]: edge){
            auto [x, y] = p;
            if (w == -1) continue;
            modified.pb({w, ct[{x, y}], cost[{x, y}]});
            edge[{y, x}] = -1;
        }
        ll sum = 0;
        for (auto [w, c, cost] : modified) sum += w * c;

        
        vec one, two;

        for(auto [w, c, cost] : modified){ 
            while(w){ 
                (cost == 2 ? two : one).pb(devc(w, 2) * c);
                w /= 2;
            }
        }
        ...
        ll dif = sum - S;
        auto f = [&](ll X) {
            for (ll a = 0; a < len(T); ++a) {
                ll b = X - 2 * a;
                if (b < 0) continue;
                if (b >= len(O)) chmin(b, len(O) - 1);

                ll tot = O[b] + T[a];
                if (tot >= dif) return true;
            }
            return false;
        };

        auto ans = bisearch(-1, len(O) + 2 * len(T) - 2, f);
