...
int main()
{
    ...
    while (T--){
        ...
        vector<P> directed, undirected;
        rep(i, M){
            ll t, x, y;
            cin >> t >> x >> y;
            --x, --y;

            (t == 0 ? undirected : directed).pb({x, y});
        }

        Graph G(N);
        for(auto& [from, to] : directed) add_arrow(G, from, to);

        vec res = topological_sort(G);
        ...
        for(auto& [from, to] : undirected){
            if(mp[from] < mp[to]) ans.pb({from + 1, to + 1});
            else ans.pb({to + 1, from + 1});
        }


        print("YES");
        for(auto a : ans) print(a);
        for(auto [from, to] : directed) cout << from + 1 << " " << to + 1 << '\n';
