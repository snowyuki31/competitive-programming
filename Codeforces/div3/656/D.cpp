...
int main()
{
    ...
    while (T--){
        ...
        ll ans = INF;
        auto f = [&](auto&& self, ll right, ll left, char c, ll ct)->void{
            if(left - right <= 1){
                chmin(ans, ct + (S[right] != c));
                return;
            }

            ll fr = 0, bk = 0;
            for (ll i = right; i < left; ++i){
                if(i < (right + left) / 2) fr += S[i] != c;
                else bk += S[i] != c;
            }

            self(self, right, (right + left) / 2, c + 1, ct + bk);
            self(self, (right + left) / 2, left, c + 1, ct + fr);
        };
